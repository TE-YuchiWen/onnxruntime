...
_Check_return_ _Ret_notnull_ OrtStatus* ORT_API_CALL CreateStatus(OrtErrorCode code, _In_z_ const char* msg) NO_EXCEPTION ORT_MUST_USE_RESULT;
...
ORT_API_STATUS_IMPL(GetStringTensorContent, _In_ const OrtValue* value,
...
ORT_API_STATUS_IMPL(CreateValue, _In_reads_(num_values) const OrtValue* const* in, size_t num_values, enum ONNXType value_type,
                    _Outptr_ OrtValue** out);
